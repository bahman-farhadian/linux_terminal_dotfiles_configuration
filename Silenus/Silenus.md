...
- **The VPN still earns its place: it makes Hephaestus itself reachable from home.** `192.168.88.0/24 via 192.168.88.1 dev tun1` resolves, so `ssh -J <you>@192.168.88.212 <guest>` reaches a guest at work from the sofa with no cable involved. Host reachability is wider than guest routing, and the SSH hop is what spans the difference.
- The hop is worth writing down because it is the answer, not a workaround: `ssh hephaestus`, then `ssh <guest-address>` from there. `ssh -J hephaestus <guest-address>` does it in one command if you would rather not think about the intermediate shell.
