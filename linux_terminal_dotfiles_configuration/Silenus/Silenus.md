...
- It also installs `lock-keyboard-en.service`, a systemd user service that switches the keyboard to English the moment the screen locks, whatever was in use — German included — so the unlock prompt is never stuck on a layout that cannot type your password. The previous layout is recorded under `$XDG_RUNTIME_DIR` and restored on unlock, so a deliberate `DE` survives the lock rather than being discarded by it.
