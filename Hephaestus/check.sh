...
ck "isolated_network_40" "$(virsh -c qemu:///system net-list --name 2>/dev/null|grep -cx isolated_network_40)" "1"
ck "isolated autostart" "$(virsh -c qemu:///system net-info isolated_network_40 2>/dev/null|awk '/^Autostart/{print $2}')" "yes"
ck "isolated no forward" "$(virsh -c qemu:///system net-dumpxml isolated_network_40 2>/dev/null|grep -c '<forward')" "0"
ck "virbr2 address"  "$(ip -4 -br addr show virbr2 2>/dev/null|awk '{print $3}')" "10.40.0.1/24"
...
_badr=$(for s in 192.168.24.0/24 192.168.40.0/24 192.168.124.0/24 10.24.0.0/24 10.32.0.0/24 10.40.0.0/24; do ip -4 route show "$s" 2>/dev/null; done \
        | sed -n 's/.*via \([0-9.]*\).*/\1/p' | grep -vxE '192.168.124.6' | sort -u | tr '\n' ' ')
